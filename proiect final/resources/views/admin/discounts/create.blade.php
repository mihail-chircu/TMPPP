...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Add Discount</h1>
            <p class="mt-1 text-sm text-gray-500">Create a new product discount.</p>
        </div>
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3 rounded-lg border border-gray-200 bg-gray-50 px-4 py-2">
                        <span id="original-price" class="text-sm text-gray-400 line-through">0.00 MDL</span>
                        <svg class="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/>
                        </svg>
                        <span id="discounted-price" class="text-sm font-bold text-green-700">0.00 MDL</span>
                    </div>
                </div>

                <div>
                    <label for="starts_at" class="mb-1 block text-sm font-medium text-gray-700">Starts At <span class="text-red-500">*</span></label>
                    <input type="datetime-local" name="starts_at" id="starts_at" value="{{ old('starts_at') }}" required class="input-field w-full rounded-lg border border-gray-300 px-4 py-2 text-sm focus:border-kinder-500 focus:outline-none focus:ring-1 focus:ring-kinder-500">
                    @error('starts_at')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="ends_at" id="ends_at" value="{{ old('ends_at') }}" required class="input-field w-full rounded-lg border border-gray-300 px-4 py-2 text-sm focus:border-kinder-500 focus:outline-none focus:ring-1 focus:ring-kinder-500">
                    @error('ends_at')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
